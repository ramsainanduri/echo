...
separately as per-base files generated by tools such as `samtools depth` or
mosdepth.
...
echo-bpsd call-cnv \
  ...
  --pon pon.pkl \
  --output sample.echo.json \
  --cnv-output sample.cnv.tsv \
  --plot sample.cnv.png
```
...
| `--output` | Yes | `.json` or `.tsv` path | Main ECHO report. JSON is recommended for complete structured output; TSV is supported for simple tabular export. |
...
- `--plot`: CNV diagnostic plot with absolute copy number, PON z-scores, PDS
  ratios, rolling median, and Bayesian breakpoint annotations
